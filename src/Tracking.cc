...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const cv::Mat & externPose, const double &timestamp)
{
    mImGray = im;
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        mCurrentFrame = Frame(mImGray, timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth, externPose);
    else
        mCurrentFrame = Frame(mImGray, timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth, externPose);
